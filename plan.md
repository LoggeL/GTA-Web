...
Last updated: 2026-07-17 08:31 CEST
Plan status: Active
Current focus: M8 — retain a real-device 30 FPS performance record from a named representative mid-range Android phone; all implementation, release, desktop, emulated-mobile, browser, soak, and publication gates are complete
...
- [ ] Measure and pass stable 30 FPS on a named representative mid-range physical Android phone; retain device, browser, renderer, average FPS, p95 frame time, worst ordinary chunk transition, quality level, and actor/residency diagnostics.
...
Current limitation: the green Pixel 7 Playwright profile is viewport, DPR, user-agent, and touch emulation running on host hardware with a software WebGL renderer. It is valid responsive, interaction, bounded-pool, and long-soak evidence, but it is not physical mid-range phone performance evidence and therefore does not close the 30 FPS device target.
...
- Latest browser smoke test: Final v1.0.0 Chromium desktop/emulated-mobile matrix passed 35 applicable scenarios with 27 intentional device skips and 0 failures; Firefox and WebKit passed 1/1 each. The corrected host-based performance matrix passed 4/4 in 2.8 minutes and the complete desktop/emulated-mobile soak passed 4/4 in 41.8 minutes. Live v1.0.0 desktop and 844×390 viewport gameplay passed with exact repository-base JS/Three.js/CSS/image assets, bounded HUD/objective geometry, hardware WebGL on desktop, adaptive low quality at the mobile viewport, and zero console warnings/errors. Physical mid-range Android performance remains the only open acceptance record.
